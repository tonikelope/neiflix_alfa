...
class Handler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def send_pls(self, files):
        ...
        for x, f in enumerate(files):
            playlist += "File" + str(x + 1) + "=http://" + self.server._client.ip + ":" + str(
                self.server._client.port) + "/" + urllib.quote(f.name) + "\n"
    ...
    def do_HEAD(self):
        ...
        if not self.server._client.file or urllib.unquote(url)[1:].decode('utf8') != self.server._client.file.name:
            for f in self.server._client.files:
                if f.name == urllib.unquote(url)[1:].decode('utf8'):
                    self.server._client.file = f
                    break

        if self.server._client.file and urllib.unquote(url)[1:].decode('utf8') == self.server._client.file.name:
            range = False
            self.offset = 0
